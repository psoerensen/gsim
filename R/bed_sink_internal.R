...
.gsim_bed_sink_append <- function(
  ...
  info_h1 <- .gsim_packed_info(h1)
  info_h2 <- .gsim_packed_info(h2)
  ...
  invisible(.Call(C_gsim_packed_bed_sink_append, sink$pointer, h1, h2))
...
.gsim_bed_sink_info <- function(sink) {
  ...
  .Call(C_gsim_packed_bed_sink_info, sink$pointer)
}
...
.gsim_bed_sink_finalize <- function(sink) {
  ...
  invisible(.Call(C_gsim_packed_bed_sink_finalize, sink$pointer))
  sink$finalized <- TRUE
  ...
  manifest <- list(
    ...
    metadata = list(
      ...
      dosage = "H1 + H2 alternative-allele count encoded directly",
...
.gsim_bed_sink_cancel <- function(sink) {
  ...
  invisible(.Call(C_gsim_packed_bed_sink_cancel, sink$pointer))
  sink$cancelled <- TRUE
  invisible(sink)
}

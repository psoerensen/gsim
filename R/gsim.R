...
gsim <- function(
  ...
  if (mode == "Glist") {
    ...
    chr_map <- stats::setNames(rep(NA_character_, length(marker_ids)), marker_ids)
  }
  ...
  out <- list(
    ...
    component = stats::setNames(component, marker_ids),
    A = A,
...
#' Print a gsim object
#'
#' @param x A `gsim` object.
#' @param ... Unused.
#'
#' @return `print.gsim()` returns `x` invisibly.
#' @method print gsim
#' @export
